...
btnImportar.addEventListener('click', async () => {
    ...
    try {
        ...
        for (const row of rows) {
            const nombre = row['Nombre del Médico']; // Extraer nombre de la columna "Nombre del Médico"
...
function renderTable(medicos) {
    ...
    medicos.forEach(medico => {
        ...
        let fecha;
        if (medico.fecha instanceof Timestamp) {
            fecha = medico.fecha.toDate().toLocaleDateString('es-CL');
        } else if (typeof medico.fecha === 'string') {
            // Intentar formatear cadenas de fecha
            fecha = new Date(medico.fecha).toLocaleDateString('es-CL');
        } else {
        ...
        row.innerHTML = `
            <td>${medico.id}</td>
            <td>${medico.nombre}</td> <!-- Solo el nombre -->
