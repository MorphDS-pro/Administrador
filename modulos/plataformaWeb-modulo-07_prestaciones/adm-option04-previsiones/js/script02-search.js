...
const getDoctors = async (queryText = '') => {
    const doctorsRef = collection(db, "previsiones");
    ...
    try {
        ...
        currentPageDoctors.forEach(doctor => {
            addRowToTable(doctor.id, doctor.nombre, doctor.fechaCreacion, doctor.usuarioDigitado, doctor.docId);
        });
...
function addRowToTable(id, nombre, fechaCreacion, usuarioDigitado, docId) {
...
async function deleteDoctor(docId, nombre, row) {
    try {
        ...
        const docRef = doc(db, 'previsiones', docId);
