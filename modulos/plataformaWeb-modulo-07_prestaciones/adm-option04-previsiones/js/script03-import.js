...
async function getNextId() {
    const snapshot = await getDocs(collection(db, 'previsiones'));
...
btnImportar.addEventListener('click', async () => {
    ...
    try {
        ...
        const data = await file.arrayBuffer();S
        const workbook = XLSX.read(data, { type: 'array' });
        const firstSheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[firstSheetName];
        const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
        const headers = jsonData[0];
        const medicoIndex = headers.findIndex(header => String(header).toLowerCase() === 'Prevision');
        if (medicoIndex === -1) {
            alert("La columna 'Prevision' no se encontró en el archivo.");
            hideImportSpinner();
            return;
        }
        ...
        hideImportSpinner();
        showMessage(messageSuccess, successText, `Se han importado ${importedCount} filas con nuevas previsiones con éxito.`);
        importContainer.classList.add('hidden');
        fileInput.value = "";
    } catch (error) {
