...
function loadCompaniesRealTime() {
    ...
    onSnapshot(collection(db, 'companies'), (snapshot) => {
        ...
        pageDocs.forEach(docObj => {
            ...
            addRowToTable(company.acciones, company.id, company.nombre, company.rut, company.registradoPor, company.fechaCreacion, docObj.id);
        });
...
function addRowToTable(acciones, id, nombre, rut, usuario, fecha, docId) {
...
btnSave.addEventListener('click', async (e) => {
    ...
    const rut = registerRutInput.value.trim();
    const usuario = document.getElementById('registerUsuario').textContent;
    const fecha = new Date();
    const fechaCreacion = fecha.toLocaleDateString('es-CL');

    if (!companyName || !rut) {
        showMessage(messageWarning, warningText, 'Advertencia: Ingrese el nombre de la unidad y el RUT');
        return;
    }
