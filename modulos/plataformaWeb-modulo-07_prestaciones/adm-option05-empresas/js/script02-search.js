...
const getCompanies = async (companyQuery = '', rutQuery = '') => {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            const companyNameLower = company.empresa ? company.empresa.toLowerCase() : '';
            ...
            if (companyNameLower.includes(companyQueryLower) && rutLower.includes(rutQueryLower)) {
                companies.push({ ...company, docId: doc.id });
            }
        });
        ...
        const currentPageCompanies = companies.slice(startIndex, startIndex + itemsPerPage);
...
function addRowToTable(id, empresa, rut, fechaCreacion, usuarioDigitado, docId) {
    ...
    row.innerHTML = `
        ...
        <td>${id}</td>
        <td>${empresa || 'Nombre no disponible'}</td> <!-- Verificación de empresa -->
        <td>${rut}</td>
        <td>${fechaCreacion}</td>
