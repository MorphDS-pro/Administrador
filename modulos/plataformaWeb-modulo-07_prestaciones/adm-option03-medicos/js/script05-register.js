...
btnSave.addEventListener('click', async (e) => {
    ...
    if (!doctorName) {
        showMessage(messageWarning, warningText, 'Advertencia: Ingrese el nombre del médico');
        return;
    }
    ...
    try {
        ...
        hideSpinner();
        showMessage(messageSuccess, successText, `El médico ${doctorName} se ha registrado con éxito`);
        formRegister.reset();
...
async function deleteDoctor(docId, nombre, row) {
    try {
        ...
        hideDeleteSpinner();
        showMessage(messageSuccess, successText, `El médico ${nombre} se ha eliminado con éxito`);
