...
async function loadPage(direction = "initial") {
  ...
  if (textFiltersActive) {
    ...
    if (
      ...
      // Se puede mantener el orderBy para otras condiciones, pero ordenaremos client-side por incrementalId.
    ...
    // Ordenar los documentos por incrementalId (o id) de forma ascendente
    ...
    try {
      ...
      // Ordenar los documentos por incrementalId de forma ascendente
      docsArray.sort((a, b) => {
        const idA = a.data().incrementalId || a.id;
        const idB = b.data().incrementalId || b.id;
        return idA.toString().localeCompare(idB.toString(), undefined, { numeric: true });
...
window.editRecord = function(id) {
  // Función para editar registros (pendiente)
};
window.deleteRecord = function(id) {
  // Función para eliminar registros (pendiente)
};
