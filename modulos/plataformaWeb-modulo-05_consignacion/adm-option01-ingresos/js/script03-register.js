import { db2, auth } from './script01-firebase-consignacion.js';
...
async function displayData() {
    ...
    const codigosRef = collection(db2, 'codigos');
        ...
        pageRecords.forEach(item => {
            ...
            row.innerHTML = `
                <td>
                    ...
                    <i class="fas fa-info-circle info-icon" data-id="${docId}" style="margin-right: 10px; cursor: pointer; color: #3498db2;"></i>
                </td>
                <td>${data.id}</td>
...
function attachRowEvents() {
    ...
    editIcons.forEach(icon => {
        icon.addEventListener('click', async (e) => {
            ...
            const docRef = doc(db2, 'codigos', docId);
...
async function deleteRecord() {
    ...
    try {
        ...
        await deleteDoc(doc(db2, 'codigos', docToDeleteId));
...
async function generateNewID() {
    const querySnapshot = await getDocs(collection(db2, 'codigos'));
